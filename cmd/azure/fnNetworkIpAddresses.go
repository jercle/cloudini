...
func listAllTenantIpAddresses(token MultiAuthToken) IPAddressList {
	...
	for _, sub := range allSubs {
		wg.Add(1)
		go listAllSubscriptionVnets(sub.SubscriptionID, token, vnets, &wg)
	}
...
func listAllSubscriptionVnets(subscriptionId string, mat MultiAuthToken, out chan<- Vnet, wg *lib.WaitGroupCount) {
	var (
		allVnets  []Vnet
...
func getAllVnetIPAddresses(mat MultiAuthToken, vnet Vnet, publicIps chan<- IPAddressItem, privateIps chan<- IPAddressItem, wg *lib.WaitGroupCount) {
	urlString := "https://management.com/subscriptions/" +
		vnet.SubscriptionID +
		"/resourceGroups/" +
		vnet.ResourceGroup +
	...
	for _, sn := range subnets {
		...
		if len(ipConfigs) > 0 {
			for _, conf := range ipConfigs {
				...
				confUrl := "https://management.com" + confId + "?api-version=2023-02-01"
				...
				if conf.Properties.PublicIpAddress != nil {
					// Is a public IP
					pubAddressUrl := "https://management.com" + conf.Properties.PublicIpAddress.ID + "?api-version=2023-02-01"
