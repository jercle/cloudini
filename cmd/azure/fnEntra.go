...
func ListAllTenantPIMScheduleInstances(token *lib.AzureMultiAuthToken) (assignmentsSlice []RoleAssignmentScheduleInstance, eligibilitiesSlice []RoleEligibilityScheduleInstance) {
	...
	// response["eligibile"] = eligibilities

	// fmt.Println("Total assignments:", strconv.Itoa(len(assignments)))
	// fmt.Println("Total eligibilities:", strconv.Itoa(len(eligibilities)))
	// os.Exit(0)

	// jsonStr, _ := json.MarshalIndent(response, "", "  ")
	// os.WriteFile("./zListAllTenantPIMScheduleInstances.json", jsonStr, os.ModePerm)
