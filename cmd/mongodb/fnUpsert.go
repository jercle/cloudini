...
func InsertCitrixMonitorData(machineData map[string]citrix.MonitorMachine, collMachines *mongo.Collection) (results *mongo.BulkWriteResult) {
	ctx := context.TODO()

	var updates []mongo.WriteModel

	for _, m := range machineData {
		// updates = append(updates, mongo.NewUpdateOneModel().SetDocument(m))
		filter := bson.D{{"_id", m.ID}}
		update := bson.D{{"$set", m}}
		updates = append(updates, mongo.NewUpdateOneModel().SetFilter(filter).SetUpdate(update).SetUpsert(true))
	}
	resMachines, err := collMachines.BulkWrite(ctx, updates, nil)
	lib.CheckFatalError(err)
