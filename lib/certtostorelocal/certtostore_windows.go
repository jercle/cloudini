...
const (
	...
	certStoreEnterprise     = uint32(certStoreEnterpriseID << compareShift)   // CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE
	certStoreCurrentUserID  = 1                                               // CERT_SYSTEM_STORE_CURRENT_USER_ID
	certStoreLocalMachineID = 2                                               // CERT_SYSTEM_STORE_LOCAL_MACHINE_ID
	certStoreEnterpriseID   = 9                                               // CERT_SYSTEM_STORE_LOCAL_MACHINE_ENTERPRISE _ID
...
// findCert wraps the CertFindCertificateInStore call. Note that any cert context passed
// into prev will be freed. If no certificate was found, nil will be returned.
func findCerts(store windows.Handle, prev *windows.CertContext) ([]*windows.CertContext, error) {
	h, _, err := certEnumCertificatesInStore.Call(
		uintptr(store),
		uintptr(unsafe.Pointer(prev)),
	)

	var list []*windows.CertContext
	...
	list = append(list, (*windows.CertContext)(unsafe.Pointer(h)))

	// count := 0

	ptr := (*windows.CertContext)(unsafe.Pointer(h))

	// for count < 10 {
	for ptr != nil {
		h2, _, _ := certEnumCertificatesInStore.Call(
			uintptr(store),
		...
		ptr = (*windows.CertContext)(unsafe.Pointer(h2))
		// if (ptr) == nil {
		// 	break
		// }
		// count++
...
func (w *WinCertStore) Certs(store string) ([]*x509.Certificate, error) {
	var storeWide *uint16
	switch store {
	case "MY":
		storeWide = wide("MY")
	case "CA":
	...
	case "ROOT":
		storeWide = wide("ROOT")
	default:
		err := fmt.Errorf("Only 'MY', 'CA', 'ROOT' stores allowed")
		lib.CheckFatalError(err)
	}

	c, err := w.certs(storeWide, w.storeDomain())
	// my   = wide("MY")
	// ca   = wide("CA")
	// root = wide("ROOT")
	if err != nil {
...
// cert is a helper function to lookup certificates based on a known issuer.
// store is used to specify which store to perform the lookup in (system or user).
func (w *WinCertStore) certs(searchRoot *uint16, store uint32) ([]*x509.Certificate, error) {
	h, err := w.storeHandle(store, searchRoot)
	if err != nil {
		return nil, err
	}
	...
	// pass 0 as the third parameter because it is not used
	// https://msdn.microsoft.com/en-us/library/windows/desktop/aa376064(v=vs.85).aspx
	certsPointers, err := findCerts(h, nil)
	if err != nil {
		return nil, fmt.Errorf("finding certificates: %v", err)
	}

	for _, ptr := range certsPointers {
		xc, err := certContextToX509(ptr)
		lib.CheckFatalError(err)
		list = append(list, xc)
	}
