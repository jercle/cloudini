// main-costExports.go - modifying with channel pipelines
package main
...
func main() {
	...
	ccdJson, _ := json.MarshalIndent(combinedCostData.rows, "", "  ")
...
func combineCostExportData(dataPath string) costExportData {
	...
	for _, file := range filePaths {
		...
		go func() {
			...
			costExportData.rows = append(costExportData.rows, data.rows...)
			mutex.Unlock()
		}()
	}
	wg.Wait()
...
func transformCostData(data costExportData) transformedCostItemsByTenant {
	...
	// 	Tenant           string
	// }
	for _, costData := range data.rows {
...
func getCostExportFileData(fileName string) (costExportData, error) {
	...
	for {
		...
		if rowData.ResourceGroup != "ResourceGroup" {
			costExport.rows = append(costExport.rows, rowData)
		}
	}
