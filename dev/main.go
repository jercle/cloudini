...
func main() {
	...
	reqVars := azureAuthRequirements{
		...
		AZURE_RESOURCE_NAME:   true,
	}
	GetAzureEnvVariables(reqVars)
...
func GetAzureEnvVariables(requiredEnvVars azureAuthRequirements) (*azureAuthDetails, error) {
	...
	// var envs azureAuthDetails

	envVarValues := reflect.ValueOf(envs)
	envVarTypes := envVarValues.Type()

	requiredValues := reflect.ValueOf(requiredEnvVars)
	// requiredValTypes := requiredValues.Type()
	for i := 0; i < envVarValues.NumField(); i++ {
		// fmt.Println(envVarTypes.Field(i).Index[0], envVarTypes.Field(i).Name, envVarValues.Field(i))
		if envVarValues.Field(i).String() == "" && requiredValues.Field(i).Bool() {
			log.Fatal(envVarTypes.Field(i).Name + " has not been assigned")
		}
	}

	// if envs.AZURE_TENANT_ID == "" ||
	// 	envs.AZURE_SUBSCRIPTION_ID == "" ||
	// 	envs.AZURE_CLIENT_ID == "" ||
	// 	envs.AZURE_CLIENT_SECRET == "" ||
	// 	envs.AZURE_RESOURCE_GROUP == "" ||
	// 	envs.AZURE_RESOURCE_NAME == "" {
	// 	if envs.AZURE_TENANT_ID == "" {
	// 		slog.Error("AZURE_TENANT_ID environment variable not set")
	// 	}
	// 	if envs.subscriptionId == "" {
	// 		slog.Error("AZURE_SUBSCRIPTION_ID environment variable not set")
	// 	}
	// 	if envs.clientId == "" {
	// 		slog.Error("AZURE_CLIENT_ID environment variable not set")
	// 	}
	// 	if envs.clientSecret == "" {
	// 		slog.Error("AZURE_CLIENT_SECRET environment variable not set")
	// 	}
	// 	if envs.resourceGroup == "" {
	// 		slog.Error("AZURE_RESOURCE_GROUP environment variable not set")
	// 	}
	// 	if envs.resourceName == "" {
	// 		slog.Error("AZURE_RESOURCE_NAME environment variable not set")
	// 	}
	// 	return nil, fmt.Errorf("Missing Environment Variable - See above error")
	// }
